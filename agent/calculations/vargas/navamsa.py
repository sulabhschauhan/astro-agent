...
EPHEMERIS NOTE (Session 52 migration): _calc_graha(jd_ut, planet, pid,
flags) delegates to helpers/ephemeris.py's sidereal_position() rather
than a direct swe.calc_ut() call. This also closes a dormant retrograde
bug: the module's own flags previously omitted FLG_SPEED, so `retrograde`
was always False for the 7 non-node grahas (same bug class as
chart_calculator.py's Session 51 FLG_SPEED fix) -- sidereal_position()
always requests FLG_SPEED, so real grahas now carry a correct retrograde
flag. No test asserted retrograde==False for a real graha (only Rahu/
Ketu's hardcoded True was tested), so this is a correctness fix with no
