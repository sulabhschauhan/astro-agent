"""
scripts/probe_pass3_preflight.py

Pre-flight smoke probe for Ring 3 pass 3 (S67 close-out). THROWAWAY
script, NOT a scoring pass -- Ring 3 pass 3 itself still requires
fresh uploads through the real Streamlit app with live human
checkpoints (CLAUDE.md "T4 golden semantics" / "Palm human checkpoint"
locks); this script exists only to catch a wiring/pipeline defect in
the S67 R1->R3->R2 landed work (per-feature retrieval, deterministic
support gate + Python-owned decline, exemplar-echo guard) BEFORE
spending a fresh-upload pass-3 run on it.

Uses the data/test_images/ fixtures -- sanctioned for mechanical
probes only (per the instructing prompt), never a substitute for Ring
3's fresh-upload requirement. This script also bypasses the palm
...
Runs the Run-C shape (hardest case, per Ring 3 pass-2 precedent):
...
introspection. The raw per-feature retrieval map (pre-gate), the
first draft's Ring 1 failures (when a retry fires), and the
per-validator breakdown this probe reports are not otherwise exposed
by PalmReadingResult's public fields, so this script uses a
capturing-client injection (generate_palm_reading's existing
test-only `client` seam) rather than reimplementing any pipeline
logic.
...
def main() -> None:
    lines: list[str] = []
    lines.append("# Pre-flight smoke probe for Ring 3 pass 3 (S67 close-out)")
    lines.append("")
    lines.append(
        "**THROWAWAY SCRIPT. NOT A SCORING PASS.** Ring 3 pass 3 itself "
        ...
        "pipeline defect in the S67 R1->R3->R2 landed work before "
        "spending a fresh-upload pass-3 run on it. Vision descriptions "
    ...
    try:
        ...
        # unsupported_features/context_corpus (from the retrieval/gate
        # section above), which should match generate_palm_reading()'s
        # internal computation deterministically (same input
        # descriptions) but is not literally the same object -- reported
        ...
        lines.append("## retry_used + first-draft Ring 1 failures")
        lines.append("")
        ...
        if result.retry_used and len(draft_texts) >= 1:
            first_draft_failures = tuple(
                palm_reading._run_ring1_checks(
                    draft_texts[0], context_corpus, unsupported_features
                )
            )
            lines.append(
                "First-draft failures (best-effort recomputation against "
                "this probe's own locally-computed unsupported_features/"
                "context_corpus -- see caveat above; this is what "
                "triggered the retry):"
            )
            lines.append("")
            lines += _fmt_validator_breakdown(first_draft_failures)
        else:
            lines.append("N/A -- first draft passed Ring 1 cleanly, no retry fired.")
        lines.append("")
        ...
        lines.append("## reading_text (verbatim, final -- includes decline block + disclaimer)")
        lines.append("")
        lines.append("```")
        ...
        lines.append("")
        lines.append(
            "All 3 sanity asserts PASSED. This is a wiring smoke check "
            "only -- it says nothing about interpretive quality/citation "
            "accuracy (that is Ring 3 pass 3's job, on fresh uploads, "
