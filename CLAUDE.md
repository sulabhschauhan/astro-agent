...
- **Sun Ayana Bala doubling** — `_ayana_bala()`'s Sun `*2.0` case has no upper clamp, producing up to ~118 Virupa, which flows into Chesta Bala and Ishta/Kashta unclamped. Impact: `compute_ishta_kashta()` guards with `max(0, ...)`, affects 3/4 reference charts. Fix: needs a cross-oracle-checked scoping decision (V1.1), not yet attempted. Session 32 DO-NOT-re-investigate lock LIFTED (Session 46 decision) — investigation reopened using the Drik Bala back-solve methodology. Pre-gate: JHora-vs-AstroSage convergence check on Moon/Venus/Sun Ayana from existing fixtures before any formula work. Sun Ayana doubling/inversion (~40 Virupa via Sun Chesta) is the primary target; Moon/Venus ±5-6 secondary.
