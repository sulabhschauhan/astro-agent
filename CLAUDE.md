...
**Session 60 CLOSED: arudha_lagna golden-set coverage + harness whitelist + q3 ratified REFUSAL.** Next: open.
...
- **arudha_lagna Stage 1 unreachable for single-mention questions** (Session 59) — score 0.333 < floor even for the literal domain name ("what is my arudha lagna") — every route is a live Stage 2 LLM call. Scorecard evidence logged 2026-07-10T17:55; candidate fixes (`_BUILT_MODULE_FASTPATH` entry vs multi-word exact-name floor exemption) are scorecard-gated, do NOT tune preemptively. STILL HELD (Session 60): the golden set now accrues this evidence directly (`sulabh_arudha_q2_stage2`/`q3_refusal_probe` exercise the single-keyword-miss and zero-keyword-miss paths respectively) per the Session 44 "tune only with Answer Scorecard evidence" lock — revisit with dogfood/scorecard data, not preemptively.
