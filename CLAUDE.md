...
**Thin-slice answer pipeline CHECKPOINT CLOSED (Session 45, 2026-07-03): orchestrator.py wires router→profile→formatter→merge. 16 e2e tests (4 career + 4 dasha + 1 marriage + 5 refusal + 2 error). Dasha conditional demotion bug fixed. 1680 passed, 3 skipped, 3 xfailed. Next: POST-CHECKPOINT PHASE ORDER per Master Build Plan (Drik extraction → ephemeris consolidation → timing block).**
...
- **Shadbala Drik Bala** — stubbed at 0.0 for all planets (BPHS 27.26 kernel diverges from AstroSage, root cause unresolved). Delta: -20.44 to +22.15 Virupa/planet (AstroSage fixture range). Impact: ratio may read "above minimum" when true ratio is below; rank usually preserved. Fix: `__drik_bala_calc_1_pvr` kernel located Session 42, not yet actioned.
- **Bhava Drishti Bala** — stubbed at 0.0 V1 (same unresolved kernel as Drik Bala, applied to Bhava Madhya instead of planet longitude). Fix: attempt together with Drik Bala's V1.1 fix — same root cause.
- **Sun Ayana Bala doubling** — `_ayana_bala()`'s Sun `*2.0` case has no upper clamp, producing up to ~118 Virupa, which flows into Chesta Bala and Ishta/Kashta unclamped. Impact: `compute_ishta_kashta()` guards with `max(0, ...)`, affects 3/4 reference charts. Fix: needs a cross-oracle-checked scoping decision (V1.1), not yet attempted.
...
- **Kala Bala Sun cross-chart (Surbhi)** — ±59 Virupa uncertainty envelope (Jupiter/Saturn Abda/Masa divergence), vs. the general 20 Virupa V1 constant used for other charts. Full detail archived in SESSION_LOG.md.
