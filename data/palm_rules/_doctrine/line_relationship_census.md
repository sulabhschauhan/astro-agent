...
**Location sensitivity:** the doctrine's CLAIM is gated by WHICH line does the stopping (Heart → ruined affections; Head → thwarted by stupidity), not by a third-party mount location — so location-sensitivity here is really "which line," already captured by the `relation_target` slot if this were wired at all (it isn't).

**Current capture: UNCAPTURED.** This is the single most consequential mis-route found in this census: instance #1 sits at the EXACT SAME termination point as `FT_016`'s positive convergence doctrine (Fate terminating at/near Heart), but today's schema can only ask "did Fate's line TERMINATE at Heart" (scalar `Position`/`TERMINATION`, `relation_types`: directional) — it has no way to distinguish "terminated because stopped/blocked" (bad) from "terminated because it joined and continued" (good, `Convergence`). A vision call reporting `TERMINATION: Line of Heart` today is doctrinally AMBIGUOUS between these two opposite-valence readings, and no rule exists (or safely could exist) keying on plain `TERMINATION → Line of Heart` for this reason — `FT_016` sidesteps the ambiguity entirely by keying on `Convergence` instead, silently leaving the "stopped by" half of the SAME sentence pair unauthorable.
...
1. **Fate "stopped by" Heart/Head** (§2, page_ref 164) — UNCAPTURABLE: no field distinguishes a negative halt-termination from the positive `Convergence` reading of the exact same endpoint; authoring a rule on plain `TERMINATION → Line of Heart`/`Line of Head` today would be doctrinally ambiguous (could mean either outcome) — this is why no such rule exists despite the doctrine being fully legible in the text.
